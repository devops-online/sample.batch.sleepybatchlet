 ...
 *
 * It uses the BatchRuntime JobOperator to start, stop, restart, and get status
 * of jobs.
 */
...
public class JobOperatorServlet extends HttpServlet {
	...
	protected static void log(String method, Object msg) {
		System.out.println("JobOperatorServlet: " + method + ": " + String.valueOf(msg));
		// logger.info("JobOperatorServlet: " + method + ": " +
		// String.valueOf(msg));
	}
	...
	/**
	 * @response help text - a list of actions understood by the servlet along
	 *           with examples.
	 */
	protected void help(HttpServletRequest request, HttpServletResponse response) throws IOException {

		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK).println("help:")
				.println("joboperator?action=help")
				.println(
						"joboperator?action=start&jobXMLName={jobXMLName}&jobParameters={name=value}&jobParameters={name=value}...")
				.println(
						"joboperator?action=restart&executionId={execId}&restartParameters={name=value}&restartParameters={name=value}...")
				...
				.println("joboperator?action=getStepExecutions&executionId={execId}").endResponse();
	}
	...
	protected void status(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		strBuffer.append("<XML><JobInstance><InstanceID>").append(jobInstance.getInstanceId()).append("</InstanceID>");
		strBuffer.append("<ExecutionID>").append(jobExecutions.get(0).getExecutionId()).append("</ExecutionID>");
		strBuffer.append("<JobName>sleepy-batchlet</JobName>");
		strBuffer.append("<Status>").append(jobExecutions.get(0).getBatchStatus()).append("</Status>");
		strBuffer.append("</JobInstance></XML>");
		stream.write(strBuffer.toString().getBytes());
		if (stream != null) {
			stream.close();
		}
	...
	protected void stop(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
	...
	protected void restart(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK).println(
				"restart(executionId=" + execId + ", restartParameters=" + restartParameters + "): Job restarted!");
	...
	protected void getJobExecution(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getJobExecution(executionId=" + execId + "): ").printJobExecution(jobExecution).endResponse();
	}
	...
	protected void getJobExecutions(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		List<JobExecution> jobExecutions = getJobOperator().getJobExecutions(new JobInstance() {
			...
			public String getJobName() {
				return "dummy-for-call-to-getJobExecutions";
			}
		});

		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getJobExecutions(instanceId=" + instanceId + "): ").printJobExecutions(jobExecutions)
				.endResponse();
	}
	...
	protected void getJobInstanceCount(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getJobInstanceCount(jobName=" + jobName + "): " + count).endResponse();
	}
	...
	protected void getJobInstance(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getJobInstance(executionId=" + execId + "): ").printJobInstance(jobInstance).endResponse();
	}
	...
	protected void getJobInstances(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				...
				.printJobInstances(jobInstances).endResponse();
	}
	...
	protected void getJobNames(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getJobNames(): ").printJobNames(jobNames).endResponse();
	}
	...
	protected void getParameters(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getParameters(executionId=" + execId + "): ").printProps(props).endResponse();
	}
	...
	protected void getRunningExecutions(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getRunningExecutions(jobName=" + jobName + "): ").printJobExecutions(jobExecutions)
				.endResponse();
	}
	...
	protected void getStepExecutions(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		getResponseWriter().setHttpServletResponse(response).beginResponse(HttpServletResponse.SC_OK)
				.println("getStepExecutions(executionId=" + execId + "): ").printStepExecutions(stepExecutions);
	}
	...
	protected String getRequiredParm(HttpServletRequest request, String queryParmName) throws IOException {
		...
		if (StringUtils.isEmpty(queryParmValue)) {
			// throw new IllegalArgumentException("ERROR: " + queryParmName + "
	 ...
	 * /joboperatorservlet?action=start&jobXMLName=sleepy-batchlet&jobParameters
	 * ={name}={value}&jobParameters={name}={value}&...
	 *
	 * @param queryParmName
	 *            The name of the query parm containing job params
	 *            ("jobParameters" or "restartParameters")
	 *
	 * @return the given query parms as a Properties object, or null if no parms
	 *         were specified.
	 */
...
class TextWriter {
	...
	public String jobInstanceToString(JobInstance jobInstance) {
		return "instanceId=" + jobInstance.getInstanceId() + ", jobName=" + jobInstance.getJobName();
	}
	...
	public String jobExecutionToString(JobExecution jobExecution) {
		return "executionId=" + jobExecution.getExecutionId() + ", jobName=" + jobExecution.getJobName()
				+ ", batchStatus=" + jobExecution.getBatchStatus() + ", createTime=" + jobExecution.getCreateTime()
				+ ", startTime=" + jobExecution.getStartTime() + ", endTime=" + jobExecution.getEndTime()
				+ ", lastUpdatedTime=" + jobExecution.getLastUpdatedTime() + ", jobParameters="
	...
	public String stepExecutionToString(StepExecution stepExecution) {
		return "stepExecutionID=" + stepExecution.getStepExecutionId() + ", stepName=" + stepExecution.getStepName()
				+ ", exitStatus=" + stepExecution.getExitStatus() + ", batchStatus=" + stepExecution.getBatchStatus()
				+ ", startTime=" + stepExecution.getStartTime() + ", endTime=" + stepExecution.getEndTime()
...
class HtmlWriter extends TextWriter {
	...
	public TextWriter printJobName(String jobName) throws IOException {
		...
		print(", <a href="
				+ StringUtils.enquote("joboperator?action=getJobInstances&jobName=" + jobName + "&start=0&count=100")
				+ ">" + "getJobInstances(jobName=" + jobName + ", start=0, count=100)" + "</a>");
		print(", <a href=" + StringUtils.enquote("joboperator?action=getJobInstanceCount&jobName=" + jobName) + ">"
				+ "getJobInstanceCount(jobName=" + jobName + ")" + "</a>");
		print(", <a href=" + StringUtils.enquote("joboperator?action=getRunningExecutions&jobName=" + jobName) + ">"
				+ "getRunningExecutions(jobName=" + jobName + ")" + "</a>");
		return println("");
	}
	...
	public TextWriter printJobInstance(JobInstance jobInstance) throws IOException {
		...
		print(", <a href="
				+ StringUtils.enquote("joboperator?action=getJobExecutions&instanceId=" + jobInstance.getInstanceId())
				+ ">" + "getJobExecutions(instanceId=" + jobInstance.getInstanceId() + ")" + "</a>");
		return println("");
	}
	...
	public TextWriter printJobExecution(JobExecution jobExecution) throws IOException {
		...
		print(", <a href="
				+ StringUtils.enquote("joboperator?action=getJobInstance&executionId=" + jobExecution.getExecutionId())
				+ ">" + "getJobInstance(executionId=" + jobExecution.getExecutionId() + ")" + "</a>");
		print(", <a href="
				+ StringUtils.enquote("joboperator?action=getParameters&executionId=" + jobExecution.getExecutionId())
				+ ">" + "getParameters(executionId=" + jobExecution.getExecutionId() + ")" + "</a>");
		print(", <a href="
				+ StringUtils
						.enquote("joboperator?action=getStepExecutions&executionId=" + jobExecution.getExecutionId())
				+ ">" + "getStepExecutions(executionId=" + jobExecution.getExecutionId() + ")" + "</a>");

		if (BatchUtils.isRunning(jobExecution.getBatchStatus())) {
			print(", <a href="
					+ StringUtils.enquote("joboperator?action=stop&executionId=" + jobExecution.getExecutionId()) + ">"
					+ "stop(executionId=" + jobExecution.getExecutionId() + ")" + "</a>");
		}
